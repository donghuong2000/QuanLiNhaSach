...
$('#changePasswordForm').submit(function (e) {
    ...
    var cpnewpass = $('#cpnewPassword').val();
    var Error = $('#validateError')
    var Success = $('#validateSuccess')
    if (current == "" || newpass == "" || cpnewpass == "") {
        Error.text("không được để trống các ô")
        e.preventDefault();
    }
    if (newpass != cpnewpass) {
        Error.text("Xác nhận mật khẩu không chính xác")
        e.preventDefault();
    }
    ...
    $.ajax({
        ...
        data: { cur: current, np: newpass, cnp: cpnewpass },
